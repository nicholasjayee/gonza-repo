...
export const useInventoryActions = (userId: string | undefined) => {
    ...
    const fetchFreshProducts = async (productIds: string[], _locationId?: string) => {
        if (!productIds.length) return [];

        console.warn('fetchFreshProducts is mocked - skipping DB fetch');
        // Return matching products from local state as fallback for now
        // This bypasses the "freshness" check but allows the action to proceed
