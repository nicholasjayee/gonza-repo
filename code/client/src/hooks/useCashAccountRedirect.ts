...
export const useCashAccountRedirect = (accounts: CashAccount[]) => {
  ...
  useEffect(() => {
    if (!hasCheckedRedirect && accounts.length > 0) {
      ...
      // Avoid cascading render warning by making the state change asynchronous
      setTimeout(() => setHasCheckedRedirect(true), 0);
    }
  }, [accounts, hasCheckedRedirect, router, setHasCheckedRedirect]);
