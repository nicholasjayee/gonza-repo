...
import { useCarriageInwards, CarriageInward } from '@/hooks/useCarriageInwards';
...
export const useProfitLossData = (
  ...
  const profitLossData = useMemo((): ProfitLossData => {
    ...
    if (dateFilter === 'specific' && specificDate) {
      ...
      from = range.from || new Date(); // Fallback to avoid undefined
    ...
    const filteredCarriageInwards = (carriageInwards || []).filter((record: CarriageInward) => {
    ...
    const totalCarriageInwards = filteredCarriageInwards.reduce((sum: number, record: CarriageInward) => {
