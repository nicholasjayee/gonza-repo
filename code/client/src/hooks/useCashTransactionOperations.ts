...
import { getCashTransactionByIdAction } from '@/app/inventory/expenses/actions';
...
export const useCashTransactionOperations = () => {
  ...
  const createInstallmentPaymentWithCash = useCallback(async (
    ...
    selectedCashAccountId: string,
    locationId: string,
    createInstallmentPayment: (data: { saleId: string; amount: number; accountId?: string; saleDescription?: string; locationId?: string; date?: Date }) => Promise<unknown>
  ) => {
    if (amount <= 0) {
      return null;
    }
  ...
  const findCashTransactionForSale = useCallback(async (cashTransactionId: string) => {
    try {
      const result = await getCashTransactionByIdAction(cashTransactionId);
