...
import { getInventoryExpensesAction } from '@/app/inventory/actions';
  ...
  useEffect(() => {
    const fetchExpenses = async () => {
      ...
      try {
        ...
        const result = await getInventoryExpensesAction(start, end);
        
        if (result.success && result.data) {
          const totalExpenses = (result.data as any[]).reduce((sum, expense) => sum + Number(expense.amount), 0);
