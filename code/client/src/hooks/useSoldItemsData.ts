...
import { getAllProductIdentifiersAction } from '@/app/inventory/actions';
...
export const useSoldItemsData = (
  ...
  const { data: dbInventoryMap, isLoading: inventoryLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const result = await getAllProductIdentifiersAction();
      if (!result.success || !result.data) return new Map<string, boolean>();
