...
import {
  ...
  SelectTrigger,
  SelectValue,
} from "@/inventory/components/ui/select";
import { Download, FileText, CalendarIcon, Search } from "lucide-react";

import { useSalesData } from "@/inventory/hooks/useSalesData";
import { useSoldItemsData } from "@/inventory/hooks/useSoldItemsData";
import { useMobileOptimization } from "@/inventory/hooks/useMobileOptimization";
import {
  Table,
  TableBody,
  TableCell,
...
import { formatNumber, formatCashCurrency } from "@/inventory/lib/utils";
import { useToast } from "@/inventory/hooks/use-toast";
import DateRangeFilter from "@/inventory/components/sales/DateRangeFilter";
import { exportSoldItemsToCSV } from "@/inventory/utils/exportSoldItemsToCSV";
import { exportSoldItemsToPDF } from "@/inventory/utils/exportSoldItemsToPDF";
import { Skeleton } from "@/inventory/components/ui/skeleton";
...
import { getDateRangeFromFilter } from "@/inventory/utils/dateFilters";

const SoldItemsTab = () => {
  const { currentBusiness } = useBusiness();
  const mobileConfig = useMobileOptimization();
  const { sales, isLoading: salesLoading } = useSalesData("desc", 10); // Load sales with a limit of 10
  ...
  const FILTER_STORAGE_KEY = useMemo(
    () =>
      currentBusiness?.id
        ? `soldItemsFilters_${currentBusiness.id}`
        : "soldItemsFilters",
    [currentBusiness?.id],
  );
  ...
  const [specificDate, setSpecificDate] = useState<Date | undefined>(
    persistedState.specificDate,
  );
  const [selectedItem, setSelectedItem] = useState<string | null>(null);
  const [isItemSalesDialogOpen, setIsItemSalesDialogOpen] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const [showOnlyNotInInventory, setShowOnlyNotInInventory] = useState(
    persistedState.showOnlyNotInInventory,
  );
  const [showOnlyCostData, setShowOnlyCostData] = useState(
    persistedState.showOnlyCostData,
  );
  ...
  const handleExportToPDF = () => {
    ...
    if (settingsLoading) {
      toast({
        title: "Please wait",
        description:
          "Business settings are still loading. Please try again in a moment.",
        variant: "destructive",
      });
      return;
    }
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">
            Items Sold - {getPeriodDisplayText()}
          </CardTitle>
          <p className="text-sm text-muted-foreground">
            Click on an item to view all sales containing that item
            {searchQuery &&
              ` • Showing ${filteredSoldItems.length} of ${soldItems.length} items`}
            {showOnlyNotInInventory &&
              ` • Filtered to show only items not in current inventory`}
          ...
          {filteredSoldItems.length === 0 ? (
            <div className="p-8 text-center text-muted-foreground">
              {searchQuery
                ...
                : showOnlyNotInInventory
                  ? "No sold items found that are not in current inventory."
            ...
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {filteredSoldItems
                    .slice(0, mobileConfig.pageSize * 2)
                    .map((item, index) => (
                      <TableRow
                        key={index}
