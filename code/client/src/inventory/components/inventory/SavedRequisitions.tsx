...
import { Requisition } from "@/inventory/hooks/useRequisitions";
...
const generateAlternativePDF = async (
  ...
  try {
    const { generateVectorPDF } =
      await import("@/inventory/utils/generateVectorPDF");
...
const SavedRequisitions = ({
  ...
  const generateRequisitionPDF = async (requisition: Requisition) => {
    try {
      console.log(
        "Starting PDF generation for requisition:",
  ...
  const RequisitionCard = ({ requisition }: { requisition: Requisition }) => {
    ...
    const urgentItems = requisition.items.filter(
      (item) => item.urgentItem,
    ).length;
      ...
      <Card className="mb-4">
        <CardHeader className="pb-3">
          <div className="flex items-start justify-between">
            ...
            <Badge
              className={cn("text-xs", getStatusColor(requisition.status))}
            >
              {requisition.status}
            </Badge>
          </div>
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  ...
                  {format(selectedRequisition.createdAt, "PPP")}
                </p>
              </div>
              <div className="flex items-center gap-2">
                <Badge
                  className={cn(
                    "text-xs",
                    getStatusColor(selectedRequisition.status),
                  )}
                >
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {selectedRequisition.items.map((item, index) => (
                        <TableRow
                          key={index}
                          className={item.urgentItem ? "bg-orange-50" : ""}
                        >
