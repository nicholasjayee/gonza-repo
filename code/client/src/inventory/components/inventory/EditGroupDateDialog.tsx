    ...
    for (const entry of group.entries) {
      ...
      if (product && product.createdAt) {
        ...
        if (selectedDate < createdAt) {
          setValidationError(
            `New date cannot be earlier than product creation date (${format(createdAt, "PP")}) for ${product.name}.`,
          );
          return false;
        }
      }
    ...
    try {
      ...
      if (success) {
        toast({
          title: "Dates updated",
          description:
            "All entries in this invoice have been updated to the new date.",
        });
        onOpenChange(false);
      }
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            This will update the date for all {group.entries.length} items in
            this invoice. Time settings for each item will be preserved.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="bg-muted p-3 rounded-lg text-sm space-y-1">
            ...
            {group.invoice && (
              <div>
                <strong>Invoice:</strong> {group.invoice}
              </div>
            )}
            <div>
              <strong>Current Date:</strong>{" "}
              {format(new Date(group.date), "PP")}
            </div>
          </div>
        </div>
