...
import { Calendar } from "@/inventory/components/ui/calendar";
import {
  Plus,
  Trash2,
  Save,
...
const MobileStockRow = ({
  ...
  const hasDateConflict = row.productId
    ? (() => {
        ...
        if (product) {
          ...
          const productDate = new Date(
            productCreationDate.getFullYear(),
            productCreationDate.getMonth(),
            productCreationDate.getDate(),
          );
    ...
    : false;
  const conflictProduct = hasDateConflict
  ...
  return (
    <Card
      className={cn(
        "mb-4 shadow-sm",
        hasDateConflict && "border-destructive border-2",
      )}
    >
      ...
      <CardContent className="space-y-4">
        ...
        <div className="relative">
          ...
          <Input
            ...
            className={cn("w-full", hasDateConflict && "border-destructive")}
          />
          {hasDateConflict && conflictProduct && (
            <p className="text-xs text-destructive mt-1">
              Product was created on{" "}
              {format(new Date(conflictProduct.createdAt), "PPP")} - after
              selected date
            </p>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Input
              ...
              value={row.quantity || ""}
              onChange={(e) =>
                onUpdateRow(row.id, "quantity", parseFloat(e.target.value) || 0)
              }
              onKeyDown={(e) => onKeyDown(e, row.id)}
              placeholder="0"
            />
          ...
          <div>
            ...
            <Input
              ...
              value={row.price || ""}
              onChange={(e) =>
                onUpdateRow(row.id, "price", parseFloat(e.target.value) || 0)
              }
              onKeyDown={(e) => onKeyDown(e, row.id)}
              placeholder="0.00"
            />
...
const BulkStockAddTab = () => {
  const { products, updateProduct, loadProducts } = useProducts(
    10000,
  ); // Load all products for proper lookup
  const { settings } = useBusinessSettings();
  const { toast } = useToast();
  const isMobile = useIsMobile();
  const { stockHistory, loadStockHistory } = useStockHistory(user?.id);
  ...
  const validateStockDate = (date: Date): string => {
    ...
    const initialDate = new Date(
      initialStockDate.getFullYear(),
      initialStockDate.getMonth(),
      initialStockDate.getDate(),
      0,
      0,
      0,
    );
  ...
  const updateRow = useCallback(
    (id: string, field: keyof StockAddRow, value: string | number) => {
      setStockRows((prev) =>
        prev.map((row) => {
          if (row.id === id) {
            const updatedRow = {
    ...
    (product: Product) => {
      ...
      setStockRows((prev) => {
        ...
        const existingRowIndex = prev.findIndex(
          (row) => row.productId === product.id && row.id !== focusedRowId,
        );
      ...
      if (row.productId) {
        ...
        if (product) {
          ...
          if (productDate.getTime() > entryDate.getTime()) {
            errors.push(
              `Row ${index + 1}: Product "${product.name}" was created on ${format(productCreationDate, "PPP")} which is after the selected stock date`,
            );
          }
        }
      }
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      variant="outline"
                      className="gap-2 justify-start text-left font-normal w-full"
                    >
                  ...
                  >
                    <Calendar
                      mode="single"
                      selected={stockEntryDate}
                      onSelect={(date) => date && setStockEntryDate(date)}
                      disabled={(date) => {
                        // Find the earliest product creation date from selected products
                        const selectedProducts = stockRows
                        ...
                        if (selectedProducts.length === 0)
                          return (
                            date < new Date("2000-01-01") || date > new Date()
                          );
                        const earliestCreationDate = selectedProducts.reduce(
                          (earliest, product) => {
                            const productDate = new Date(product!.createdAt);
                            return !earliest || productDate < earliest
                              ? productDate
                              : earliest;
                          },
                          null as Date | null,
                        ...
                        return (
                          date <
                            (earliestCreationDate || new Date("2000-01-01")) ||
                          date > new Date()
                        );
                      }}
                      className={cn(
                        "p-3 pointer-events-auto",
                        validateStockDate(stockEntryDate) &&
                          "border-destructive",
                      )}
                      initialFocus
                    />
              ...
              <div className="flex space-x-2">
                ...
                <Button
                  ...
                  disabled={isLoading || stockRows.length === 0}
                  className="gap-2 flex-1 md:flex-none"
                >
                  <Save size={16} />
            ...
            <div className="p-4 space-y-4">
              {stockRows.map((row, index) => (
                <MobileStockRow
                  ...
                  isLastRow={index === stockRows.length - 1}
                  ...
                  {stockRows.map((row, index) => {
                    ...
                    const hasDateConflict = row.productId
                      ? (() => {
                          ...
                          if (product) {
                            const productCreationDate = new Date(
                              product.createdAt,
                            );
                            const selectedDate = new Date(stockEntryDate);
                            const productDate = new Date(
                          ...
                          <div className="relative">
                            ...
                            {hasDateConflict && conflictProduct && (
                              <p className="text-xs text-destructive mt-1">
                                Product was created on{" "}
                                {format(
                                  new Date(conflictProduct.createdAt),
                                  "PPP",
                                )}{" "}
                                - after selected date
                              </p>
                        ...
                        <TableCell>
                          <div className="flex gap-1">
                            ...
                            <Button
                              ...
                              disabled={index === stockRows.length - 1}
                              className="h-8 w-8"
                            >
                              <ChevronDown size={14} />
                            </Button>
                  ...
                  <TableRow className="border-none">
                    <TableCell colSpan={5} className="py-1 text-center">
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          addRowAfter(stockRows[stockRows.length - 1].id)
                        }
