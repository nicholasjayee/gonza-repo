...
import {
  ...
  PaginationNext,
  PaginationPrevious,
} from "@/inventory/components/ui/pagination";
...
const StockSummaryTable: React.FC<StockSummaryTableProps> = ({
  ...
  const [categoryFilter, setCategoryFilter] = useState(
    persistedFilters.categoryFilter,
  );
  ...
  const [stockStatusFilter, setStockStatusFilter] = useState(
    persistedFilters.stockStatusFilter,
  );
  ...
  const isStockMismatch = (item: StockSummaryData): boolean => {
    ...
    return currentProduct
      ? currentProduct.quantity !== item.closingStock
      : false;
  };
  ...
  const filteredAndSortedData = useMemo(() => {
    const filtered = data.filter((item) => {
      ...
      if (stockStatusFilter !== "all") {
        if (stockStatusFilter === "in-stock" && item.closingStock <= 0)
          return false;
        if (stockStatusFilter === "out-of-stock" && item.closingStock > 0)
          return false;
        if (
          stockStatusFilter === "low-stock" &&
          (item.closingStock > 10 || item.closingStock <= 0)
        )
          return false;
      }
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case "closingStock":
          ...
          break;
        case "revaluation":
          comparison = a.revaluation - b.revaluation;
          break;
      }
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="min-w-[150px]">
          <Select
            ...
            onValueChange={handleStockStatusFilterChange}
          >
            <SelectTrigger>
        ...
        <div className="flex items-center gap-4">
          <Badge variant="secondary" className="text-xs">
            ...
            {filteredAndSortedData.length === 1 ? "product" : "products"} found
          </Badge>
          {totalPages > 1 && (
            ...
            <TableRow>
              ...
              <TableHead className="text-right">
                <Button
                  ...
                  className="h-auto p-0 font-medium text-right justify-end hover:bg-transparent"
                >
                  Items Sold {getSortIcon("itemsSold")}
                </Button>
              </TableHead>
              <TableHead className="text-right">
                <Button
              ...
              <TableHead className="text-right">
                <Button
                  ...
                  className="h-auto p-0 font-medium text-right justify-end hover:bg-transparent"
                >
                  Transfer Out {getSortIcon("transferOut")}
                </Button>
              </TableHead>
              <TableHead className="text-right">
                <Button
                  variant="ghost"
                  onClick={() => handleSort("returnIn")}
                  className="h-auto p-0 font-medium text-right justify-end hover:bg-transparent"
                >
                  Return In {getSortIcon("returnIn")}
                </Button>
              </TableHead>
              <TableHead className="text-right">
                <Button
                  variant="ghost"
                  onClick={() => handleSort("returnOut")}
                  className="h-auto p-0 font-medium text-right justify-end hover:bg-transparent"
                >
                  Return Out {getSortIcon("returnOut")}
                </Button>
              </TableHead>
              <TableHead className="text-right">
                <Button
              ...
              <TableHead className="text-right">
                <Button
                  variant="ghost"
                  onClick={() => handleSort("revaluation")}
                  className="h-auto p-0 font-medium text-right justify-end hover:bg-transparent"
                >
                  Revaluation {getSortIcon("revaluation")}
                </Button>
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {paginatedData.map((item) => (
              ...
              >
                ...
                <TableCell className="text-right">{item.stockIn}</TableCell>
                <TableCell className="text-right">{item.transferOut}</TableCell>
                <TableCell className="text-right">{item.returnIn}</TableCell>
                <TableCell className="text-right">{item.returnOut}</TableCell>
                <TableCell className="text-right font-semibold">
                  {item.closingStock}
                </TableCell>
                <TableCell className="text-right text-muted-foreground">
                  {item.revaluation !== 0
                    ? item.revaluation.toLocaleString()
                    : "-"}
                </TableCell>
              </TableRow>
            ))}
            {filteredAndSortedData.length > 0 && (
              <TableRow className="font-semibold bg-muted/50">
                ...
                <TableCell className="text-right">{totals.stockIn}</TableCell>
                <TableCell className="text-right">
                  {totals.transferOut}
                </TableCell>
                <TableCell className="text-right">{totals.returnIn}</TableCell>
                <TableCell className="text-right">{totals.returnOut}</TableCell>
                <TableCell className="text-right">
                  {totals.closingStock}
                </TableCell>
                <TableCell className="text-right">
                  {totals.revaluation !== 0
                    ? totals.revaluation.toLocaleString()
                    : "-"}
                </TableCell>
              </TableRow>
            )}
