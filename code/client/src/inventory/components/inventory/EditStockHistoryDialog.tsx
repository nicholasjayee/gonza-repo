...
const EditStockHistoryDialog: React.FC<EditStockHistoryDialogProps> = ({
    ...
    try {
      ...
      if (success) {
        ...
        toast({
          title: "Update failed",
          description:
            "Failed to update stock history entry. Please try again.",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const isSaleRelatedEntry = (entry: StockHistoryEntry): boolean => {
    ...
    return saleReasons.some((reason) =>
      entry.changeReason.toLowerCase().includes(reason.toLowerCase()),
    );
  };
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              This is the initial stock entry. You can modify quantity, reason,
              and date, but the date must be earlier than all other stock
              transactions.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                className={
                  validationError
                    ? "border-destructive border-2 bg-destructive/5 focus:border-destructive focus:ring-destructive"
                    : ""
                }
              />
              {validationError && (
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              This stock history entry was created by a sales operation and can
              only be modified through the Edit Sale form.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            <div className="bg-muted p-3 rounded-lg space-y-2">
              ...
              <div className="text-sm space-y-1">
                <div>
                  <strong>Date:</strong> {format(entry.createdAt, "PPP p")}
                </div>
                <div>
                  <strong>Change:</strong> {entry.oldQuantity} →{" "}
                  {entry.newQuantity}
                </div>
                <div>
            ...
            <div className="bg-blue-50 border border-blue-200 p-3 rounded-lg">
              <div className="text-blue-800 text-sm">
                <strong>Note:</strong> To modify this entry, please use the Edit
                Sale form in the Sales section.
              </div>
            </div>
          </div>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Modify the details of this stock history entry. The product&apos;s
            current stock will be recalculated after saving.
          </DialogDescription>
        </DialogHeader>
