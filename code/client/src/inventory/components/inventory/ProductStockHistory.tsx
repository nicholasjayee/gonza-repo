...
// Extend the type to include properties that might be missing or added at runtime
...
interface ProductStockHistoryProps {
  stockHistory: ExtendedStockHistoryEntry[];
  ...
  const displayItems = useMemo(() => {
    ...
    const items = [
      ...
      ...Object.values(saleGroups).map((group) => {
        ...
        const sortedForMain = [...group].sort(
          (a, b) =>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    {hasHistory ? (
                      <>
                        ...
                        >
                          Original: {originalRef.oldQuantity} →{" "}
                        ...
                        >
                          Current: {currentRef.oldQuantity} →{" "}
                      ...
                      <Badge
                        ...
                        className="text-xs bg-muted/50 font-normal"
                      >
                        Qty: {main.oldQuantity} → {main.newQuantity}
                      </Badge>
                    )}
                    {!hasHistory && (
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <Badge
                      ...
                      className="text-xs bg-muted/50 font-normal"
                    >
                      Qty: {entry.oldQuantity} → {entry.newQuantity}
                    </Badge>
                    <div className="text-sm text-gray-500 flex items-center gap-1 ml-2">
                      <Clock className="inline-block w-3 h-3" />
                      {format(entry.createdAt, "MMM d, yyyy h:mm a")}
                ...
                <div className="flex items-center gap-3">
                  <Button
                    ...
                    className="h-8 px-2 text-xs"
                    onClick={() =>
                      setExpandedManual(isExpanded ? null : entry.id)
                    }
                  >
                    {isExpanded ? "Hide Details" : "View Details"}
                  </Button>
                  {isInitialStockEntry(entry) ? (
                    <>
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          setEditHistoryDialog({ open: true, entry })
                        }
                        className="h-8 px-2"
                      >
                        <Edit className="h-3 w-3" />
                      </Button>
                      <div className="text-xs text-muted-foreground px-2 py-1 bg-blue-100 rounded">
                        Initial Stock
                      </div>
                    </>
                  ) : (
                    <>
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          setDeleteHistoryDialog({ open: true, entry })
                        }
  ...
  return (
    <>
      <Card>
        ...
        <CardContent>{stockHistoryList}</CardContent>
      </Card>
      ...
      <EditStockHistoryDialog
        ...
        isInitialStock={
          editHistoryDialog.entry
            ? isInitialStockEntry(editHistoryDialog.entry)
            : false
        }
          ...
          editHistoryDialog.entry
            ? (() => {
                const product = products.find(
                  (p) => p.id === editHistoryDialog.entry?.productId,
                );
