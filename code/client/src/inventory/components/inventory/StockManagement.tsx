...
const useStockHistory = () => ({
  createStockHistoryEntry: async () => true,
  stockHistory: [] as any[],
});
...
const StockManagement: React.FC<StockManagementProps> = ({
  ...
  const { createStockHistoryEntry, stockHistory } = useStockHistory();
  ...
  const validateDateTime = (dateStr: string, timeStr: string): string => {
    ...
    if (initialStockDate) {
      ...
      const initialStockDateOnly = new Date(
      ...
      if (proposedDateOnly.getTime() === initialStockDateOnly.getTime()) {
        const timeDifference =
          proposedDate.getTime() - initialStockDate.getTime();
  ...
  const handleStockIn = async () => {
    ...
    if (!quantity || quantity <= 0) {
      toast({
        ...
        description:
          "Please enter a valid positive number for stock in quantity.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleStockOut = async () => {
    ...
    if (!quantity || quantity <= 0) {
      toast({
        title: "Invalid Quantity",
        description:
          "Please enter a valid positive number for stock out quantity.",
        variant: "destructive",
      });
      return;
    }
      ...
      {initialStockDate && (
        <div className="bg-blue-50 border border-blue-200 p-3 rounded-lg">
          <div className="text-blue-800 text-sm">
            <strong>Initial Stock Date:</strong>{" "}
            {format(initialStockDate, "PPP p")}
            <br />
            <span className="text-xs">
              New stock entries must be after this date/time
            </span>
          </div>
        </div>
      )}
