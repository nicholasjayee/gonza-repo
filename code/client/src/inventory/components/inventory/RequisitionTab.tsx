...
import { useRequisitions } from "@/inventory/hooks/useRequisitions";
...
const MobileRequisitionRow = ({
  ...
  return (
    <Card
      className={cn("mb-4 shadow-sm", row.urgentItem && "border-destructive")}
    >
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Input
              ...
              value={row.quantity || ""}
              onChange={(e) =>
                onUpdateRow(row.id, "quantity", parseInt(e.target.value) || 0)
              }
              onKeyDown={(e) => onKeyDown(e, row.id)}
              placeholder="0"
            />
          ...
          <div className="flex items-end">
            <label className="flex items-center gap-2 text-xs font-medium cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  onUpdateRow(row.id, "urgentItem", e.target.checked)
                }
                className="rounded"
              />
              Mark as urgent
...
const RequisitionTab = () => {
  const { products } = useProducts(10000); // Load all products for proper lookup
  ...
  const {
    ...
    createRequisition,
    deleteRequisition,
  } = useRequisitions(undefined, currentBusiness?.id);
  ...
  const addLowStockItem = useCallback(
    (product: Product) => {
      const existingRow = requisitionItems.find(
  ...
  const generatePDF = useCallback(() => {
    ...
    doc.text(
      `Business: ${settings.businessName || "Your Business Name"}`,
      20,
      40,
    );
    ...
    const validItems = requisitionItems.filter(
    ...
    if (validItems.length === 0) {
      ...
      doc.autoTable({
        head: [
          ["#", "Item Description", "Qty", "Unit Cost", "Amount", "Urgent"],
        ],
        body: tableData,
      ...
      doc.text(
        `Total Amount: ${settings.currency} ${formatNumber(totalAmount)}`,
        20,
        yPos + 30,
      );
  ...
  const saveRequisition = useCallback(async () => {
    ...
    if (validItems.length === 0) {
      toast({
        ...
        description:
          "Please add at least one item with a quantity greater than 0.",
        variant: "destructive",
      });
      return;
    }
  ...
  const totalQuantity = requisitionItems.reduce(
        ...
        <Card className="border-orange-200 bg-orange-50">
          ...
          <CardContent>
            <p className="text-sm text-orange-700 mb-3">
              The following items are running low or out of stock. Click to add
              them to your requisition:
            </p>
            <div className="flex flex-wrap gap-2">
              {(lowStockExpanded
                ? paginatedLowStockItems
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setLowStockPage((prev) =>
                        Math.min(totalLowStockPages, prev + 1),
                      )
                    }
        ...
        <CardContent className="p-0 md:p-6">
                  ...
                  {requisitionItems.map((row, index) => {
                    ...
                    const itemAmount =
                      product && row.quantity
                        ? product.costPrice * row.quantity
                        : 0;

                    return (
                      <TableRow
                        key={row.id}
                        className={row.urgentItem ? "bg-destructive/5" : ""}
                      >
                        <TableCell>
                          <div className="relative">
                            <Input
                        ...
                        <TableCell className="text-sm font-medium">
                          {itemAmount > 0
                            ? `${settings.currency} ${formatNumber(itemAmount)}`
                            : "-"}
                        </TableCell>
                        ...
                        <TableCell>
                          <input
                            ...
                            checked={row.urgentItem || false}
                            onChange={(e) =>
                              updateRow(row.id, "urgentItem", e.target.checked)
                            }
                            className="rounded"
                          />
                        </TableCell>
                  ...
                  <TableRow className="border-none">
                    <TableCell colSpan={6} className="py-1 text-center">
                      <Button
                        ...
                        onClick={() =>
                          addRowAfter(
                            requisitionItems[requisitionItems.length - 1].id,
                          )
                        }
          ...
          {requisitionItems.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">
              No items added. Click &quot;Add Item&quot; to start creating your
              requisition.
            </div>
          )}
        </CardContent>
