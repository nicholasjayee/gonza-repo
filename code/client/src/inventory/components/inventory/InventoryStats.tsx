...
const InventoryStats: React.FC<InventoryStatsProps> = ({
  ...
  const lowStockProducts =
    typeof lowStockOverride === "number"
      ? lowStockOverride
      : products.filter((p) => p.quantity > 0 && p.quantity <= p.minimumStock)
          .length;
  const outOfStockProducts =
    typeof outOfStockOverride === "number"
      ? outOfStockOverride
      : products.filter((p) => p.quantity === 0).length;

  const totalInventoryValue =
    typeof totalCostValueOverride === "number"
      ? totalCostValueOverride
      : products.reduce(
  ...
  const totalStockValue =
    typeof totalStockValueOverride === "number"
      ? totalStockValueOverride
  ...
  return (
    <div
      className={`grid gap-3 md:gap-4 mb-6 ${hideStockValue ? "grid-cols-1 sm:grid-cols-2 md:grid-cols-4" : "grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5"}`}
    >
        ...
        <CardContent className="p-4 md:p-6">
          <div className="flex items-center">
            ...
            <div className="ml-3 md:ml-4 min-w-0 flex-1">
              ...
              <h3
                className={`${getDynamicFontSize(`${settings.currency} ${formatNumber(totalInventoryValue)}`)} font-bold break-all leading-tight`}
              >
                {settings.currency} {formatNumber(totalInventoryValue)}
              </h3>
            </div>
          </div>
        </CardContent>
          ...
          <CardContent className="p-4 md:p-6">
            <div className="flex items-center">
              ...
              <div className="ml-3 md:ml-4 min-w-0 flex-1">
                ...
                <h3
                  className={`${getDynamicFontSize(`${settings.currency} ${formatNumber(totalStockValue)}`)} font-bold break-all leading-tight`}
                >
                  {settings.currency} {formatNumber(totalStockValue)}
                </h3>
              </div>
            </div>
          </CardContent>
