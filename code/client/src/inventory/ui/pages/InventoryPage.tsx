...
export default function InventoryPage({ branchType, branches = [] }: InventoryPageProps) {
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'products' | 'analysis' | 'restocks' | 'requisitions' | 'stock-taking'>('products');
    ...
    const handleExportAnalysis = (formatType: 'excel' | 'pdf') => {
        ...
        const data = [
            ...
            { Metric: 'Out of Inventory Sales', Value: `${analysis.outOfStockSales} Items` },
        ...
        if (formatType === 'excel') {
            ...
            XLSX.writeFile(wb, `inventory_analysis_${format(new Date(), 'yyyyMMdd_HHmm')}.xlsx`);
        } else {
            const doc = new jsPDF();
    ...
    async function loadData(filterBranchId?: string | null) {
        ...
        try {
            const [overviewRes, movementsRes, analysisRes, requisitionsRes] = await Promise.all([
            ...
            if (movementsRes.success) setMovements(movementsRes.data as InventoryMovement[]);
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center py-20 animate-pulse">
            ...
            <p className="text-sm font-bold text-muted-foreground">Analyzing inventory data...</p>
        </div>
    );
        ...
        <div className="space-y-8 max-w-6xl mx-auto pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground font-medium">Real-time stock tracking and business intelligence</p>
                </div>
            ...
            <div className="animate-in fade-in duration-500">
                {activeTab === 'products' && (
                    <ProductsTab
                        ...
                        categories={[]}
                        currency="UGX"
                        onViewProduct={(id) => router.push(`/inventory/products/${id}`)}
                        onEditProduct={(id) => router.push(`/inventory/products/${id}/edit`)}
                    />
                )}
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="bg-card border border-border p-8 rounded-[2rem] flex items-center justify-between">
                                <div>
                                    <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-60 mb-1">In Inventory Sales</p>
                                    <h4 className="text-3xl font-black italic">{analysis.inStockSales} Items</h4>
                                </div>
                            ...
                            <div className="bg-card border border-border p-8 rounded-[2rem] flex items-center justify-between">
                                <div>
                                    <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-60 mb-1">Out of Inventory Sales</p>
                                    <h4 className="text-3xl font-black italic text-rose-500">{analysis.outOfStockSales} Items</h4>
                                </div>
                        ...
                        <div className="bg-blue-500/5 border border-blue-500/10 p-6 rounded-2xl flex items-start gap-4">
                            ...
                            <p className="text-xs font-medium text-blue-700/80 leading-relaxed italic">
                                This analysis identifies sales made when stock levels were zero or unavailable.
                                Consistently high "Out of Inventory" sales indicate potential lost revenue or inaccurate tracking.
                            </p>
                        </div>
                    </div>
                )}
                            ...
                            <table className="w-full text-left font-sans">
                                <thead>
                                    <tr className="bg-muted/30 border-b border-border">
                                        ...
                                        <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground">Change</th>
                                        <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground">User</th>
                                        <th className="px-8 py-5 text-right text-[10px] font-black uppercase tracking-widest text-muted-foreground">Date</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-border/50">
                                    {movements.filter((m: any) => m.type === 'RESTOCK' || m.type === 'ADJUSTMENT').map((movement: any) => (
                                        <tr key={movement.id} className="hover:bg-muted/20 transition-colors group">
                                            <td className="px-8 py-5 flex items-center gap-4">
                                                ...
                                                <span className="font-bold text-sm">{movement.productName}</span>
                                            </td>
                                            <td className="px-6 py-5">
                                                <span className={`px-3 py-1 rounded-full text-[10px] font-black ${movement.quantityChange > 0 ? 'bg-emerald-500/10 text-emerald-600' : 'bg-rose-500/10 text-rose-500'}`}>
                                                    {movement.quantityChange > 0 ? '+' : ''}{movement.quantityChange} Units
                                                </span>
                                            </td>
                                            <td className="px-6 py-5 text-sm font-medium text-muted-foreground">{movement.userName}</td>
                                            <td className="px-8 py-5 text-right">
                                                <div className="flex flex-col text-right">
                                                    <span className="text-xs font-bold">{format(new Date(movement.createdAt), 'MMM dd, yyyy')}</span>
                                                    <span className="text-[10px] text-muted-foreground uppercase">{format(new Date(movement.createdAt), 'hh:mm a')}</span>
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
                ...
                {activeTab === 'requisitions' && (
                    <div className="space-y-6">
                        ...
                        <RequisitionList requisitions={requisitions} onRefresh={() => {
...
function TabButton({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string }) {
    return (
        <button
            onClick={onClick}
            className={`px-6 py-4 flex items-center gap-2 text-xs font-black uppercase tracking-widest transition-all relative ${active ? 'text-primary' : 'text-muted-foreground hover:text-foreground'
                }`}
        >
            {icon}
            {label}
