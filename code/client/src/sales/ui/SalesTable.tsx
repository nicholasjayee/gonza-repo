...
import { BusinessSettings } from '@/inventory/types/index';
import { Table, TableBody, TableHead, TableHeader, TableRow } from '@/inventory/components/ui/table';
...
import SalesCategoryManager from './SalesCategoryManager';
...
import { useCashTransactions } from '../../inventory/hooks/useCashTransactions'; 
...
import { Edit, Trash2, Receipt, FileText, Quote, Heart } from 'lucide-react';
import { formatNumber } from '@/lib/utils';
import BusinessNoticeGenerator from "@/inventory/components/customers/BusinessNoticeGenerator";
import { generatePaymentReminderPDF } from "@/sales/utils/generatePaymentReminderPDF";
...
const MobileCard = React.memo(({
  ...
  // Use a map instead of switch function for icons
  ...
  const ButtonIcon = IconMap[sale.paymentStatus] || Receipt;
      ...
      <CardContent className="p-4">
          ...
          <div className="flex-1 min-w-0 pr-2">
            ...
            {sale.paymentStatus === 'Installment Sale' && actualAmountDue > 0 && (
              <Badge variant="outline" className="text-xs bg-red-50 text-red-700 border-red-200 mt-1">
                Due: {settings.currency} {formatNumber(actualAmountDue)}
              </Badge>
            )}
          </div>
          <span
        ...
        <div className="space-y-3 mb-3">
          ...
          <div className="flex justify-between items-center">
            <div className="text-left">
              ...
              <p className="font-semibold text-gray-900">{settings.currency} {formatNumber(totalCost)}</p>
            </div>
            {totalDiscount > 0 ? (
              <div className="text-right">
                <p className="text-xs text-gray-500">Discount</p>
                <p className="font-semibold text-orange-600">-{settings.currency} {formatNumber(totalDiscount)}</p>
              </div>
            ) : (
              <div className="text-right">
                <p className="text-xs text-gray-500">Discount</p>
          ...
          <div className="flex justify-between items-center">
            <div className="text-left">
              ...
              <p className="font-semibold text-gray-900">{settings.currency} {formatNumber(totalWithTax)}</p>
              {taxRate > 0 && (
                <p className="text-xs text-gray-500">Tax incl.</p>
              )}
              {sale.paymentStatus === 'Installment Sale' && actualAmountPaid > 0 && (
                <p className="text-xs text-green-600 mt-1">
                  Paid: {settings.currency} {formatNumber(actualAmountPaid)}
                </p>
              )}
            </div>
            <div className="text-right">
              <p className="text-xs text-gray-500">Profit</p>
              <p className="font-semibold text-green-600">{settings.currency} {formatNumber(totalWithTax - totalCost)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="pt-3 border-t border-gray-100 space-y-2">
          <div className="flex flex-wrap gap-2">
            ...
            >
              <ButtonIcon className="h-3 w-3 shrink-0" />
...
const SalesTable: React.FC<SalesTableProps> = ({
  ...
  const getCashAccountName = useCallback((sale: Sale) => {
    ...
    const linkedTransaction = transactions.find((transaction: { id: string; accountId: string }) => transaction.id === sale.cashTransactionId);
  ...
  const handleSendPaymentReminder = useCallback(async (sale: Sale) => {
    try {
      ...
      toast({
        title: "Success",
        description: "Payment reminder generated successfully.",
      });
    } catch (error) {
      console.error("Error generating payment reminder:", error);
      toast({
        title: "Error",
        description: "Failed to generate payment reminder.",
        variant: "destructive"
      });
    }
  }, [settings, toast]);
      ...
      <div className="bg-white rounded-lg shadow-sm border border-border/50">
        ...
        <div className="p-4 sm:p-6">
          ...
          <SalesTableFilters
            ...
            setCashTransactionFilter={(val: string) => setCashTransactionFilter(val as "all" | "linked" | "unlinked")}
      ...
      {selectedSaleForNotice && (
        <BusinessNoticeGenerator
          customer={{
              ...
              phoneNumber: selectedSaleForNotice.customerPhone,
