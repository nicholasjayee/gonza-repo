...
const useBusiness = () => ({ currentBusiness: { name: 'Mock Business' } });
const useBusinessSettings = () => ({ settings: { businessPhone: '123-456-7890' } });
const useMessages = (userId: string) => ({ templates: [], isLoading: false });
...
const SaleFormActions: React.FC<SaleFormActionsProps> = ({
  ...
  const thankYouTemplates = useMemo(() => {
    return templates.filter(
      (t: any) => t.category && String(t.category).trim() === "ThankYou"
    );
  }, [templates]);
  ...
  useEffect(() => {
    ...
    if (selectedTemplateId === 'default') {
      ...
      const tpl = thankYouTemplates.find((t: any) => t.id === selectedTemplateId);
      if (tpl) {
  ...
  const handleTemplateChange = (id: string) => {
    ...
    if (id === 'default') {
      ...
      const tpl = thankYouTemplates.find((t: any) => t.id === id);
      if (tpl) {
