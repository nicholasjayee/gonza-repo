...
import { getExpensesAction } from '@/expenses/api/controller';
      ...
      <div className="bg-white p-3 border border-gray-200 shadow-sm rounded-md">
        ...
        {payload.map((entry: any, index: number) => (
          <p key={`tooltip-${index}`} className="text-sm">
            <span className="font-medium" style={{ color: entry.stroke }}>{entry.name}: </span>
...
const SalesPerformanceChart: React.FC<SalesPerformanceChartProps> = ({ 
  ...
  const [expensesData, setExpensesData] = useState<Array<{date: string | Date, amount: number}>>([]);
  ...
  useEffect(() => {
    const fetchExpenses = async () => {
      try {
        ...
        // Determine start and end dates based on filters
        if (isCustomRange && dateRange.from && dateRange.to) {
        ...
        const result = await getExpensesAction({
            ...
            filterBranchId: currentBusiness.id
        });
        
        if (result.success && result.data) {
           // Map to format needed. The data from server action is cleaner.
           setExpensesData(result.data.map((e: any) => ({
  ...
  const prepareChartData = () => {
    ...
    if (timeFrame === 'weekly') {
      ...
      // const weekEnd = endOfWeek(now, { weekStartsOn: 1 }); // Sunday - unused
