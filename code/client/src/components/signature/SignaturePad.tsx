...
const SignaturePadComponent: React.FC<SignaturePadComponentProps> = ({
  ...
  useEffect(() => {
    if (canvasRef.current && showPad) {
      ...
      const handleEndStroke = () => {
        setIsDrawing(false);
        setHasSignatureState(!signaturePad.isEmpty());
      };
  ...
  useEffect(() => {
    // Initial check
    if (signaturePadRef.current) {
        setHasSignatureState(!signaturePadRef.current.isEmpty());
    }
  }, [showPad]); // Check when pad visibility changes
