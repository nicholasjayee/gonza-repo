...
// The type definition for capacitor-bluetooth-spp might be missing addListener
const BluetoothPlugin = BluetoothSpp as any;

BluetoothPlugin.addListener('connected', (info: any) => console.log('Event: connected', info));
BluetoothPlugin.addListener('connectionFailed', (info: any) => console.log('Event: connectionFailed', info));
BluetoothPlugin.addListener('disconnected', (info: any) => console.log('Event: disconnected', info));
BluetoothPlugin.addListener('deviceFound', (device: any) => console.log('Event: deviceFound', device));
BluetoothPlugin.addListener('discoveryFinished', () => console.log('Event: discoveryFinished'));
...
export async function printBluetooth(data: string) {
  ...
  try {
    ...
    try { await BluetoothSpp.disconnect(); } catch { /* ignore */ }
